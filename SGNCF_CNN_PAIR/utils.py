...
def get_norm(model, type):
    ...
    if type == 'emb':
        ...
        for name, param in model.named_parameters():
            name_part = name[:2]
            if name_part == 'cn' or name_part == 'fc':
                params.append(param)
...
def plt_evalution(epochs, recalls, mrrs, k, alpha, lr_emb, l2_emb, lr_gcn, l2_gcn, model_type,
    ...
    # plot title
    plt.title(model_type+'-Alpha'+str(alpha)+'_'+str(lr_emb)+'_'+str(l2_emb)+'_'+str(lr_gcn)+'_'+str(l2_gcn)+'_'+str(lr_cnn)+'_'+str(l2_cnn))

    # save
    plt.savefig(model_type+'-Alpha'+str(alpha)+'_'+str(lr_emb)+'_'+str(l2_emb)+'_'+str(lr_gcn)+'_'+str(l2_gcn)+'_'+str(lr_cnn)+'_'+str(l2_cnn)+'.png')

    # close
    plt.close(fig)

def plt_norm(epochs, emb_norms, gcn_norms, cnn_norms, alpha, lr_emb, l2_emb, lr_gcn, l2_gcn, model_type,
             lr_cnn, l2_cnn):
    fig = plt.figure()
    ...
    plt.plot(epochs, cnn_norms, c='blue', label='cnn_norm')
    ...
    # plot title
    plt.title(model_type+'-Norm_Alpha' + str(alpha)+'_'+str(lr_emb)+'_'+str(l2_emb)+'_'+str(lr_gcn)+'_'+str(l2_gcn)+'_'+str(lr_cnn)+'_'+str(l2_cnn))

    # save
    plt.savefig(model_type+'-Norm_Alpha' + str(alpha)+'_'+str(lr_emb)+'_'+str(l2_emb)+'_'+str(lr_gcn)+'_'+str(l2_gcn)+'_'+str(lr_cnn)+'_'+str(l2_cnn)+'.png')
